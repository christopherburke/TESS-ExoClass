...
#                   (allvet == 1) & (alltic == 12862119 ) & (allpn==1))[0]
    ...
    for i, curTic in enumerate(alltic):
        ...
        if np.mod(i, nWrk) == wID:
            curPn = allpn[i]
            outFile = os.path.join(make_data_dirs(sesMesDir, SECTOR, curTic), 'tess_mods_diffImg_{0:016d}_{1:02d}_{2:02d}.pdf'.format(curTic,curPn,SECTOR))
            if OVERWRITE or not (os.path.isfile(outFile)):
                ...
                fileInput = os.path.join(make_data_dirs(sesMesDir, SECTOR, curTic), 'tess_tpf_{0:016d}.h5d'.format(curTic))
                f = h5py.File(fileInput, 'r')
                ...
                #plt.colorbar()    
                #plt.show()
                usePhase = phaseData(time, allper[i], allatepoch[i])
                phaseDur = alldur[i] / 24.0 / allper[i]
                useEvents = assignEvents(time[validData], allatepoch[i], usePhase[validData], allper[i], phaseDur)
                eventTime = usePhase[validData] + useEvents
                ...
                idx = np.where(np.logical_not(validData))[0]
                ootvdGd[idx] = False
        ...
        #                    plt.show()
                        ...
                        fileOutput = os.path.join(make_data_dirs(sesMesDir, SECTOR, curTic), 'tess_cent_modtemp.txt')
                        np.savetxt(fileOutput, np.transpose([time[validData], \
                                                final_smooth_flux[validData]-1.0, \
                                                trpzdModel]))
                        pngOutputPrefix = os.path.join(make_data_dirs(sesMesDir, SECTOR, curTic), 'tess_cnt_modtemp_{0:02d}_{1:02d}'.format(ii,jj))
                        # Build argument list
