...
if __name__ == '__main__':
    ...
    dirOutputs = '/pdo/users/cjburke/spocvet/sector61/'
    RESAMP = 31  ###  USE AN ODD NUMBER ###
    SECTOR = 61# =-1 if multi-sector

    if (not 'maxsec' in locals()) and SECTOR == -1:
        print('Code expects multi-sector (SECTOR = 61) but no sector argument was given. EXITING!')
        sys.exit()
    ...
    if SECTOR > 0:
        ...
        fileInputPrefixList.append('/pdo/spoc-data/sector-061/light-curve/tess2023018032328-s0061-')
        fileInputSuffixList = []
        for i in np.arange(1,SECTOR):
            fileInputSuffixList.append('/foo{0:d}'.format(i))
        fileInputSuffixList.append('-0250-s_lc.fits.gz')
    else:
        print('Multi-Sector')
    ...
    #fileOut = 'spoc_pdcstats_sector-61_20230302.txt'
    #fom = open(fileOut, 'w')
